...
First, create the vocab file

```
python preprocess.py -train_src X --train_tgt Y --share_vocab True --save_data some_dir/the_vocab
```

Second, create the training file per task/domain/source_language/target_language
```
...
Third, create a json file that lists all of the training files and the task/domains/input language/output languages
...
"SOURCE_language-TASK-DOMAIN-TARGET_LANGUAGE":"file_location.train",
  ...
  "vocab":"file_location"
  "valid-DE-TRANSLATE-COMPLEX-DE":"file_location.train"
}
...
To train a model 
```
python  train.py -data  some_dir\something.json  -layers 6 -rnn_size 512 -word_vec_size 512 -transformer_ff 8 -heads 12         -encoder_type transformer -decoder_type transformer -position_encoding         -train_steps 200000  -max_generator_batches 2 -dropout 0.1         -batch_size 20 -batch_type tokens -normalization tokens  -accum_count 1         -optim adam -adam_beta2 0.998 -decay_method noam -warmup_steps 8000 -learning_rate 2         -max_grad_norm 0 -param_init 0  -param_init_glorot         -label_smoothing 0.1 -valid_steps 1 -save_checkpoint_steps 1 --share_embeddings
```
...
## Predicition 
```
python translate.py --length_penalty avg --beam_size 30  --replace_unk --dynamic_dict --share_vocab  --gpu 0 --model model_location.pt  -v --threshold 0.2  --ctags SIMPLY-SIMPLY-SIMPY-SIMP-DE  --batch_size 20  --max_length 250 --src input_file --output output_location 
```
```
