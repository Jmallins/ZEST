...
def _feature_tokenize(
    ...
    if layer >= len(tokens[0].split(feat_delim)):
        layer = len(tokens[0].split(feat_delim))-1
...
class TextMultiField(RawField):
    ...
    def process(self, batch, device=None):
        ...
        if  True or self.maxl is None:
            maxl = len(batch_by_feat)

        feats = [ff.process(batch_by_feat[i], device=device)
                 for i, (_, ff) in enumerate(self.fields[1:maxl], 1)]
